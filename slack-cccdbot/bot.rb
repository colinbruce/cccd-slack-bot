  ...
  class Bot < SlackRubyBot::Bot
    help do
      ...
      command 'branch' do
        ...
        long_desc 'Shows the ping branch page for CCCD environment(s), accepts multiples '\
                  'e.g.  `cccd branch dev and api-sandbox` or `cccd branch dev, staging and production`'
      end

      command 'details' do
        desc '`cccd details <environment>` e.g. `cccd details staging`'
        long_desc 'Shows the ping details page for CCCD environment(s), accepts multiples '\
                  'e.g.  `cccd details dev and api-sandbox` or `cccd details dev, staging and production`'
      end
    ...
    SlackRubyBot.configure do |config|
      config.aliases = %w[cccdbot getpaidbot cb]
    end
  end
end
